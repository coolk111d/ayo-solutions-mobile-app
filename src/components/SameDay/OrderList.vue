<template>
    <ion-header>
        <ion-toolbar>
            ...
            <ion-title>Same Day Transactions</ion-title>
        </ion-toolbar>  
    </ion-header>
            ...
            <ion-row class="ion-align-items-center" v-for="order of orders" :key="order.id">
                    <ion-col size="12" style="text-align:center;">
                         <!-- Product Title --><a class="product-title" href=""><span style="color:#feb041; font-size: 14px;">Order #{{order.tracking_number}}</span></a>
                            ...
                            <p class="sale-price">Total Amount: <span class="price">&#8369;{{order.total_price_with_tax}}</span></p>       
...
export default defineComponent({
    ...
    methods : {
        ...
        orderDetails(id) {
            modalController.dismiss();
            this.router.push(`/sameday-order-details`);
        },
        async getOrders() {
